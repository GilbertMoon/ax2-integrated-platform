...
def render_context_exception(request, exc):
    ...
    if isinstance(exc, NoActiveRound):
        ...
        return render(request, "idea/integration/no_round.html")
    if isinstance(exc, RoundSelectionRequired):
        return render(request, "idea/integration/select_round.html", {"rounds": exc.rounds})
    if isinstance(exc, PermissionDenied):
        return render(request, "403.html", status=403)
    if isinstance(exc, IntegrationError):
        logger.exception("Integration context could not be resolved")
        return render(request, "idea/integration/unavailable.html", status=503)
    raise exc
...
def round_context(request: HttpRequest):
    ...
    try:
        ...
        return render(
            request,
            "idea/integration/select_round.html",
            {"rounds": exc.rounds},
        )
    except PermissionDenied:
        return render(request, "403.html", status=403)
    ...
    return render(
        request,
        "idea/integration/context_ready.html",
