...
로그인 후 우측 상단 프로필 메뉴의 **마이페이지 → 바로 아래 오류 신고**를 누르면 신고 내역이 먼저 표시됩니다.
내역 화면의 **새 오류 신고**에서 제목, 오류 내용, 발생 페이지, 캡처(선택)를 제출합니다.
캡처는 파일 선택 또는 Ctrl+V 붙여넣기를 지원하며 PNG/JPEG/WebP, 최대 5MB·2천만 화소로 제한합니다.
사용자는 본인 신고만 조회하며 상세 화면에서 첨부한 캡처와 접수 완료 / 해결 완료 상태를 확인합니다.
기존 운영 권한을 가진 승인된 튜터·관리자는 전체 신고를 조회하고 두 상태 중 하나로 변경합니다.
