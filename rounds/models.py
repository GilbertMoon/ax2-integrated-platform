...
class EvaluationRound(models.Model):
    ...
    # 최종 점수 반영 비율(%) - 네 값의 합은 항상 100이어야 한다(clean()·DB 제약 둘 다 검증).
    # 기본값 40/60/0/0은 LMS와 튜터 점수를 안 쓰는 기존 계산식과 같아 이전 회차 동작을 보존한다.
    ...
    lms_score_weight = models.PositiveSmallIntegerField(default=0)
    ...
    class Meta:
        ...
        constraints = [
            ...
            models.CheckConstraint(
                ...
                & Q(lms_score_weight__gte=0, lms_score_weight__lte=100),
    ...
    def clean(self):
        ...
        weight_total = (
            self.team_score_weight
            + self.personal_score_weight
            + self.tutor_score_weight
            + self.lms_score_weight
        )
        if weight_total != 100:
            errors["team_score_weight"] = (
                f"팀·개인·튜터·LMS 점수 비율의 합은 100%여야 합니다 (현재 {weight_total}%)."
            )
        if errors:
