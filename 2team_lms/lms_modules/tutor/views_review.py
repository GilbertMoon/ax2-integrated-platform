...
def _review_url(pk, src) -> str:
    url = reverse("lms:tutor:submission-review", args=[pk])
    qs = _carry_qs(src)
...
def submission_review(request, pk):
    ...
    return render(
        request,
        "lms_ui/tutor/review_panel.html",
        {
...
def ai_evaluation_generate(request, pk):
    ...
    try:
        ...
        if not exc.links:
            ...
            hint = (
                "GitHub 저장소·폴더 링크에서 읽을 코드를 확보하지 못했습니다. "
    ...
    if result.unreadable_links:
        extra = (
            " (저장소·폴더 링크는 파일 목록만 확인됩니다. 파일 페이지 링크나 파일 첨부로 다시 확인해 주세요)"
