...
from lms_modules.common.preview import (  # noqa: F401
...
from lms_modules.common.preview import (
    _preview,
)
...
from lms_modules.tutor import github_fetch, grading
...
def assignment_list(request):
    ...
    # 마감 지남 + 미제출 + 지각 불가(allow_late=False) — 제출은 여전히 막혀 있지만
    # 목록에서까지 사라지면 안 된다("제출 불가"와 "목록 숨김"은 별개). late_rows와
    # 마찬가지로 "해야 할 과제" 탭 아래 별도 섹션으로만 노출한다 — 필터/집계/제출 로직은 무변경.
    closed_rows = [
        row
        for row in rows
        if row["tab_status"] == "closed" and not row["is_late_available"] and matches_controls(row)
    ]
    closed_rows.sort(key=lambda row: (row["assignment"].due_at, row["assignment"].id), reverse=True)

    page_obj = Paginator(filtered_rows, 10).get_page(request.GET.get("page"))
    page_rows = list(page_obj.object_list)
    return render(
        request,
        "lms_ui/student/assignment_list.html",
        {
            "rows": page_rows,
...
def assignment_submit(request, assignment_id):
    ...
    if assignment.is_team and team is None:
        ...
        return redirect("lms:student:assignment-list")
    ...
    if request.method == "POST" and form.is_valid():
        ...
        if resource_error:
            return render(request, "lms_ui/student/submission_form.html", {
                "assignment": assignment,
                "form": form,
        ...
        try:
            with transaction.atomic():
                ...
                if Submission.objects.filter(**existing_filter).exists():
                    ...
                    return redirect(
                        "lms:student:assignment-preview", assignment_id=assignment.id
                    )
...
def assignment_preview(request, assignment_id):
    ...
    return render(request, "lms_ui/student/submission_preview.html", {
        "assignment": assignment,
        "submission": submission,
        "last_editor": editor,
