...
from teams.models import Team, TeamMembership, TeamFormationSnapshot
from teams.lms_formation import calculate, selection, decode, base_scores
...
class LmsFormationTests(TestCase):
    ...
    def setUp(self):
        ...
        self.tutor = User.objects.create_user(email='formation-tutor@example.com', role=User.Role.TUTOR,
            approval_status=User.ApprovalStatus.APPROVED, must_rotate_password=False)
        self.round = EvaluationRound.objects.create(title='Next', created_by=self.tutor,
            evaluation_start_at=self.now, evaluation_end_at=self.now + timedelta(days=1), target_team_count=2)
        self.people = []
        for i in range(4):
            user = User.objects.create_user(email=f'formation-{i}@example.com', role=User.Role.STUDENT,
                approval_status=User.ApprovalStatus.APPROVED, must_rotate_password=False)
            self.people.append(RoundParticipant.objects.create(round=self.round, user=user,
                student_number_snapshot=str(i), display_name_snapshot=str(i)))
        self.assignment = Assignment.objects.create(title='Task', created_by=self.tutor.pk,
            due_at=self.now-timedelta(days=1))
        for p in self.people:
            Submission.objects.create(assignment=self.assignment, student_id=p.user_id, final_score=80)

    def chosen(self, weight=100, rid=None):
        return {'enabled':True, 'weight':weight,
            'items':[{'assignment_id':self.assignment.pk, 'round_id':rid}]}

    def post(self, name, payload):
        self.login(self.tutor)
        return self.client.post(reverse('teams:'+name, args=[self.round.pk]), payload,
    ...
    def test_existing_lms_contribution_is_removed_and_result_unchanged(self):
        old = EvaluationRound.objects.create(title='Scored', created_by=self.tutor,
            evaluation_start_at=self.now-timedelta(days=3), evaluation_end_at=self.now-timedelta(days=2),
            status='COMPLETED', completed_at=self.now-timedelta(days=1),
            team_score_weight=40, personal_score_weight=40, tutor_score_weight=0, lms_score_weight=20)
        p = RoundParticipant.objects.create(round=old, user=self.people[0].user,
            student_number_snapshot='old', display_name_snapshot='old')
        run = CalculationRun.objects.create(round=old, version=1, status='SUCCEEDED',
            is_active=True, formula_version='v1', executed_by=self.tutor)
        row = EvaluationResult.objects.create(calculation_run=run, result_type='INDIVIDUAL',
            participant=p, team_score_raw=3, peer_score_raw=4, lms_score_raw=5, final_score_raw=Decimal('3.8'))
    ...
    def test_snapshot_failure_rolls_back_team_save(self):
        response = self.post('auto-assignment', {'team_count':2, 'lock_version':0,
            'lms_selection':self.chosen()})
        self.assertEqual(response.status_code, 200, response.content)
        body = response.json()
        with patch('teams.models.TeamFormationSnapshot.objects.create', side_effect=RuntimeError('snapshot failed')):
            with patch('teams.views._notify_team_assignments') as notify:
                with self.assertRaisesMessage(RuntimeError, 'snapshot failed'):
                    self.post('save-team', {'lock_version':0, 'teams':body['teams'],
                        'formation_token':body['formation_token'], 'formation_required':True})
    ...
    def test_historical_team_submission_and_missing_membership(self):
        old = EvaluationRound.objects.create(title='Old', created_by=self.tutor,
            evaluation_start_at=self.now-timedelta(days=3), evaluation_end_at=self.now-timedelta(hours=1))
        participant = RoundParticipant.objects.create(round=old, user=self.people[0].user,
            student_number_snapshot='old', display_name_snapshot='old')
    ...
    def test_manual_board_save_keeps_signed_evidence_and_rejects_stale_token(self):
        response = self.post('auto-assignment', {'team_count':2, 'lock_version':0,
            'lms_selection':self.chosen(), 'scores_only':True})
        self.assertEqual(response.status_code, 200, response.content)
        body = response.json()
        ...
        payload = {'lock_version':0, 'formation_required':True, 'formation_token':token,
            'teams':[{'team_number':i+1, 'name':f'Team {i+1}',
                'participant_ids':[p.pk for p in self.people[i*2:i*2+2]]} for i in range(2)]}
        ...
        self.assertEqual(page.context['team_data']['formation_evidence']['selection'], snapshot.evidence['selection'])
