...
from lms_modules.core.models import Lecture, Lesson, LessonMaterial, LessonVideo
...
class StudentLectureDateGroupingTests(TestCase):
    ...
    def setUp(self):
        self.user = get_user_model().objects.create_user(approval_status="approved", is_active=True, is_onboarded=True, email="student-lecture@example.com")
    ...
    def test_detail_normalizes_target_lesson_id_to_group(self):
        ...
        # l2 로 들어와도 같은 그룹으로
        resp2 = self.client.get(reverse("lms:student:lecture-detail", args=[self.l2.id]))
        self.assertEqual(resp2.context["target_lesson_id"], self.l1.id)

    def test_list_shows_one_row_per_date_with_combined_counts(self):
        resp = self.client.get(reverse("lms:student:lecture-list"))
