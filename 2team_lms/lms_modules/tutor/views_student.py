...
def student_list(request):
    ...
    for stu in students:
        ...
        if len(ung) == 1:
            ungraded_url = reverse("lms:tutor:submission-review", args=[ung[0]])
        elif len(ung) > 1:
            ungraded_url = reverse("lms:tutor:student-detail", args=[stu.id])
        else:
            ungraded_url = None
    ...
    return render(request, "lms_ui/tutor/student_list.html", {
        "rows": rows,
...
def student_detail(request, student_id):
    ...
    return render(request, "lms_ui/tutor/student_detail.html", {
        "student": student,
        "team": team.name if team else None,
        "timeline": timeline,
        "final_score": score.final if score else None,
