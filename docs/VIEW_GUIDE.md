...
accounts_user.id
    ↓
rounds_roundparticipant.user_id

rounds_roundparticipant.id
    ↓
teams_teammembership.participant_id

teams_teammembership.team_id
    ↓
teams_team.id

rounds_roundparticipant.round_id
    ↓
...
즉, 다음과 같이 사용자의 Round별 팀 소속 변화를 확인할 수 있습니다.
...
특정 사용자의 기본 정보와 대표적으로 사용할 팀 정보를 조회할 때 사용합니다.
...
사용자가 **각 Round에서 어느 팀에 소속되었는지** 확인할 때 사용합니다.
...
이를 통해 각 팀에서 사용자-참가자-팀 소속 관계를 개별적으로 구현하는 것을 줄이고, 공통 데이터 연결 로직을 VIEW에서 관리할 수 있습니다.
...
현재 제공 VIEW는 **조회 전용 인터페이스**입니다. 팀 애플리케이션에서 VIEW를 통해 원본 데이터를 직접 수정하는 방식은 사용하지 않습니다.

### ② `round_id`를 중요 식별자로 사용

동일한 사용자가 여러 Round에 참여할 수 있으며 Round마다 다른 팀에 소속될 수 있습니다.

따라서 팀 정보를 사용할 때 다음 조합을 권장합니다.
...
`rounds_roundparticipant`의 `student_number_snapshot`, `display_name_snapshot`은 해당 Round 당시의 사용자 정보를 보존하기 위한 스냅샷입니다.

따라서 현재 `accounts_user`의 이름이나 기타 정보와 다를 수 있으며, Round별 평가/팀 화면에서는 snapshot 값을 우선 사용하는 것을 권장합니다.
...
특히 `user_round_team_view`는 다음 질문에 답하기 위한 VIEW입니다.
...
다른 팀에서 사용자와 팀 정보를 연계할 때 해당 VIEW를 공통 인터페이스로 활용할 수 있습니다.
