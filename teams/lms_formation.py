...
from results.services import calculate_seed, calculate_final_score
from teams.models import TeamMembership
from teams.django_backend import DjangoTeamsDataSource
from teams.application import create_auto_team_board, RoundNotEditableError, TeamVersionConflictError
...
def selection(value):
    ...
    for item in items:
        ...
        if type(aid) is not int or aid <= 0 or (rid is not None and (type(rid) is not int or rid <= 0)):
    ...
    return {'enabled': enabled, 'weight': weight if enabled else 0,
...
def base_scores(participants, target_round):
    ...
    for pid, uid in participants.items():
        history, used = [], []
        rows = EvaluationResult.objects.filter(result_type='INDIVIDUAL', participant__user_id=uid,
            calculation_run__is_active=True, calculation_run__round__status='COMPLETED',
            final_score_raw__isnull=False).exclude(calculation_run__round_id=target_round).select_related(
                'calculation_run__round').order_by('-calculation_run__round__completed_at', '-pk')
        for row in rows:
            ...
            if w:
                expected = calculate_final_score(
                    row.team_score_raw, row.peer_score_raw, row.tutor_score_raw, row.lms_score_raw,
                    team_weight=Decimal(rnd.team_score_weight)/100,
                    peer_weight=Decimal(rnd.personal_score_weight)/100,
                    tutor_weight=Decimal(rnd.tutor_score_weight)/100, lms_weight=w)
                if expected != row.final_score_raw:
                    raise ValidationError(f'회차 {rnd.pk}의 현재 비율과 저장된 평가 결과가 일치하지 않습니다. 과거 계산 기준을 확인해 주세요.')
            score = (row.final_score_raw - (row.lms_score_raw or Decimal(0)) * w) / (1-w)
            if not 0 <= score <= 5:
                raise ValidationError('기존 평가 점수와 비율이 일치하지 않습니다. 과거 채점 설정을 확인해 주세요.')
            history.append(score)
            used.append({'result_id': row.pk, 'round_id': rnd.pk, 'final': str(row.final_score_raw),
                         'lms': str(row.lms_score_raw), 'lms_weight': rnd.lms_score_weight, 'base': str(score)})
            if len(history) == 3:
                break
...
def calculate(round_id, raw):
    ...
    participants = dict(RoundParticipant.objects.filter(round_id=round_id).values_list('pk', 'user_id'))
    base, history = base_scores(participants, round_id)
    evidence = {'selection': chosen, 'calculated_at': timezone.now().isoformat(), 'history': history,
                'students': {}, 'policy': {}, 'assignments': []}
    ...
    evidence['policy'] = {f.name: getattr(policy, f.name) for f in policy._meta.fields if f.name not in ('id','updated_at')}
    membership = {(m.participant.round_id, m.participant.user_id): m.team_id for m in
        TeamMembership.objects.filter(participant__round_id__in=round_ids, team__round_id__in=round_ids).select_related('participant','team')
    ...
    for a in assignments:
        ...
        evidence['assignments'].append({'id': a.pk, 'title': a.title, 'round_id':rid_by_a[a.pk],
            'due_at':a.due_at.isoformat(), 'is_team':a.is_team, 'is_required':a.is_required,
            'weight_tier':a.weight_tier, 'late_penalty':a.late_penalty})
    combined = {}
    for pid, uid in participants.items():
        ...
        for a in assignments:
            ...
            if sub is not None and sub.final_score is None:
                raise ValidationError(f'미채점 제출물이 있습니다: 과제 {a.pk}, 학생 ID {uid}. 평가 후 다시 계산해 주세요.')
            applicable.append(a)
            if a.is_team and sub:
                mapped[(0,a.pk)] = sub
            details.append({'assignment_id':a.pk,'team_id':team_id,'submission_id':sub.pk if sub else None,
                            'score':sub.final_score if sub else None,'submitted_at':sub.submitted_at.isoformat() if sub else None})
        ...
        mixed = lms if w == 1 else (base[pid]*(1-w)+lms*w if base[pid] is not None and lms is not None else None)
        combined[pid] = mixed
        evidence['students'][str(pid)] = {'student_id':uid,'base':str(base[pid]) if base[pid] is not None else None,
            'lms_total':score.final,'combined':str(mixed) if mixed is not None else None,'details':details}
...
def preview(round_id, request_data, raw, actor_id, *, scores_only=False):
    ...
    assignable = tuple(p for p in current.participant_ids if p not in excluded) or current.participant_ids
    result = None if scores_only else create_auto_team_board(round_id=round_id,lock_version=current.lock_version,
        participant_ids=assignable,seed_scores=scores,team_count=request_data.team_count,
        previous_teammate_pairs=source.get_previous_teammate_pairs(round_id,assignable),rng=random.Random())
    evidence.update(round_id=round_id,lock_version=current.lock_version,actor_id=actor_id,
...
def decode(token, round_id, version, actor_id):
    try:
        ...
        raise ValidationError('점수 계산이 만료되었거나 변경되었습니다. 점수 계산 또는 자동 배치를 다시 실행해 주세요.')
    if (value['round_id'],value['lock_version'],value['actor_id']) != (round_id,version,actor_id):
...
def round_assignment_ids(round_id):
    ...
    ids = set()
    for values in RoundScore.objects.filter(round_id=round_id).values_list('assignment_ids', flat=True):
        ids.update(values)
    previous = EvaluationRound.objects.filter(evaluation_end_at__lt=rnd.evaluation_end_at).exclude(pk=round_id).order_by('-evaluation_end_at').first()
