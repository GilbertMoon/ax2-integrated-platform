...
class UserAttributeExternalIdMapper:
    ...
    def map(self, user) -> int | None:
        ...
        # 통합 플랫폼에서는 Core accounts.User의 PK가 공용 VIEW user_id와 같다.
        # 3조 standalone 어댑터의 external_user_id가 없어도 기존 로그인 사용자를
        # 그대로 매핑해 별도 OTP 계정 테이블을 만들지 않는다.
        mapped_id = external_user_id if external_user_id is not None else getattr(user, "pk", None)
