...
def management_team_page(request: HttpRequest, round_id: int) -> HttpResponse:
    ...
    snapshot = TeamFormationSnapshot.objects.filter(
        round_id=round_id, version=team_data["lock_version"]
    ).first()
...
def auto_assignment_view(request: HttpRequest, round_id: int) -> JsonResponse:
    ...
    try:
        ...
        if "lms_selection" in payload:
            ...
            result, token, evidence = preview(
                round_id,
                request_data,
                payload["lms_selection"],
                request.user.id,
                scores_only=payload.get("scores_only") is True,
            )
...
def save_team_view(request: HttpRequest, round_id: int) -> JsonResponse:
    ...
    try:
        ...
        if payload.get("formation_token"):
            ...
            evidence = decode(
                payload["formation_token"],
                round_id,
                request_data.board.lock_version,
                request.user.id,
            )
        if payload.get("formation_required") and evidence is None:
            raise ValidationError(
                "점수 계산 또는 자동 배치를 실행해 계산 근거를 먼저 확인해 주세요."
            )
        with transaction.atomic():
            board = get_teams_backend().save_team_configuration(
                round_id, request.user.id, request_data
            )
            if evidence is not None:
                from teams.models import TeamFormationSnapshot

                evidence["saved_teams"] = [
                    {"team_number": t.team_number, "participant_ids": list(t.participant_ids)}
                    for t in board.teams
                ]
                TeamFormationSnapshot.objects.create(
                    round_id=round_id,
...
def lms_formation_catalog(request, round_id):
    ...
    return JsonResponse(
        {
            "items": list(page.object_list.values("id", "title", "is_team", "due_at")),
            "page": page.number,
