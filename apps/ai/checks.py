...
def gemini_key_is_configured(app_configs, **kwargs):
    if (
        ...
        and not settings.PRD_GEMINI_API_KEY.strip()
    ):
        return [
            checks.Error(
                "Gemini AI provider is enabled without a PRD_GEMINI_API_KEY.",
                hint="Set PRD_GEMINI_API_KEY in the deployment secret store.",
                id="ai.E001",
            )
        ]
    return []
