...
from lms_modules.tutor import ai_gemini, github_fetch
...
class FetchGithubFileTests(TestCase):
    ...
    def test_success(self):
        with patch("lms_modules.tutor.github_fetch.requests.get", return_value=self._resp()):
    ...
    def test_404(self):
        with patch("lms_modules.tutor.github_fetch.requests.get", return_value=self._resp(status=404)):
            self.assertIsNone(
    ...
    def test_too_large(self):
        big = b"x" * (513 * 1024)
        with patch("lms_modules.tutor.github_fetch.requests.get", return_value=self._resp(content=big)):
            self.assertIsNone(
                github_fetch.fetch_github_file("https://github.com/o/r/blob/main/a.py")
            )

    def test_binary(self):
        with patch("lms_modules.tutor.github_fetch.requests.get", return_value=self._resp(content=b"\xff\xfe\x00")):
            self.assertIsNone(
                github_fetch.fetch_github_file("https://github.com/o/r/blob/main/a.py")
            )

    def test_network_error(self):
        with patch("lms_modules.tutor.github_fetch.requests.get", side_effect=github_fetch.requests.RequestException):
            self.assertIsNone(
                github_fetch.fetch_github_file("https://github.com/o/r/blob/main/a.py")
            )
...
class ProbeGithubFileTests(TestCase):
    """제출 시점 링크 검증(views_submit._github_link_error)이 쓰는 probe_github_file.

    "유효성 검증 실패"(not_blob·not_found)와 "일시적 API 장애"(error)는 서로 다르게
    처리돼야 한다 — 전자는 호출부가 제출을 막고, 후자는 통과시킨다.
    """

    def _get(self, status):
        return patch(
            "lms_modules.tutor.github_fetch.requests.get",
    ...
    def test_server_5xx_is_error_not_not_found(self):
        # 일시적 서버 오류는 "검증 실패"가 아니라 "판단 보류"로 통과시켜야 한다.
        with self._get(503):
            self.assertEqual(
                github_fetch.probe_github_file("https://github.com/o/r/blob/main/a.py"),
                "error",
...
class BuildPromptTests(TestCase):
    ...
    def test_local_file_counted(self):
        ...
        with patch("lms_modules.tutor.ai_gemini._read_text", return_value="def f(): return 1"):
    ...
    def test_github_link_fetched_and_counted(self):
        ...
        with patch("lms_modules.tutor.ai_gemini._read_text", return_value=None), \
             patch("lms_modules.tutor.ai_gemini.github_fetch.fetch_github_file", return_value="print('hi from gh')"):
            prompt, read_count, unreadable = ai_gemini._build_prompt(self.submission)
        self.assertEqual(read_count, 1)
        self.assertEqual(unreadable, [])
    ...
    def test_broken_github_link_only(self):
        ...
        with patch("lms_modules.tutor.ai_gemini._read_text", return_value=None), \
             patch("lms_modules.tutor.ai_gemini.github_fetch.fetch_github_file", return_value=None):
            prompt, read_count, unreadable = ai_gemini._build_prompt(self.submission)
    ...
    def test_mixed_file_plus_broken_link(self):
        ...
        with patch("lms_modules.tutor.ai_gemini._read_text", side_effect=fake_read), \
             patch("lms_modules.tutor.ai_gemini.github_fetch.fetch_github_file", return_value=None):
            prompt, read_count, unreadable = ai_gemini._build_prompt(self.submission)
        self.assertEqual(read_count, 1)
        self.assertEqual(unreadable, [url])
    ...
    def test_ipynb_link_cell_parsed(self):
        ...
        with patch("lms_modules.tutor.ai_gemini._read_text", return_value=None), \
             patch("lms_modules.tutor.ai_gemini.github_fetch.fetch_github_file", return_value=raw):
            prompt, read_count, unreadable = ai_gemini._build_prompt(self.submission)
        self.assertEqual(read_count, 1)
