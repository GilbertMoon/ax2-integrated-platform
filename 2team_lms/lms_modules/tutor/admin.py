...
class GradingPolicyAdmin(admin.ModelAdmin):
    ...
    def has_add_permission(self, request):
        # Core admin navigation must not depend on LMS database availability.
        if not request.path.startswith("/admin/tutor/gradingpolicy/"):
            return False
        return super().has_add_permission(request) and not GradingPolicy.objects.exists()
