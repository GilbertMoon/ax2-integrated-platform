...
def summary(request):
    ...
    return JsonResponse(
        {"unread_count": unread_count(request.user), "items": [_serialize(item) for item in items]}
    )
...
def slack_link_view(request):
    ...
    if not user:
        return JsonResponse(
            {"ok": False, "error": "프로젝트 사용자를 찾을 수 없습니다."}, status=400
        )
    try:
        identity = link_slack_user(
            user=user, slack_user_id=(request.POST.get("slack_user_id") or "").strip()
        )
...
def slack_send(request):
    _require_operations(request)
    slack_users = list(
        SlackIdentity.objects.filter(is_active=True)
        .select_related("user")
        .order_by("slack_display_name", "slack_user_id")
    )
    teams = []
    if request.method == "POST":
        ...
        if not title:
            ...
            identity = SlackIdentity.objects.filter(
                pk=request.POST.get("slack_identity_id"), is_active=True
            ).first()
            if not identity:
                ...
                if send_slack_dm(
                    slack_user_id=identity.slack_user_id, title=title, message=message, link=link
                ):
                    messages.success(
                        request,
                        f"{identity.slack_display_name or identity.slack_user_id}님에게 Slack DM을 보냈습니다.",
                    )
                else:
    ...
    return render(
        request, "notifications/slack_send.html", {"slack_users": slack_users, "teams": teams}
    )
